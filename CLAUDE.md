...
- `0086_newsletter_subscribers.sql` — newsletter ingest table (`newsletter_subscribers`) for the landing's "Stay in the loop" form. Pre-fix the landing posted to `/api/email-subscribe` — an endpoint the Worker never implemented; the form showed "Subscribed. Check your inbox" while no D1 row was created and no email was sent. The fix adds the Worker route handler [src/http/subscribeHttp.js](manicbot/src/http/subscribeHttp.js) (canonical path `/api/subscribe`, plus an alias `/api/email-subscribe` so the existing landing keeps working without redeploy) backed by [src/http/subscribeHttpLogic.js](manicbot/src/http/subscribeHttpLogic.js) for pure parse / lang-fold / source-allowlist. Idempotent on `email` via UNIQUE index + `INSERT OR IGNORE`; second submit is a silent no-op (no double row, no double welcome). On the NEW-row path the Worker fires a fire-and-forget POST to admin-app `POST /api/internal/newsletter-welcome` ([admin-app/src/app/api/internal/newsletter-welcome/route.ts](manicbot/admin-app/src/app/api/internal/newsletter-welcome/route.ts)) authenticated with `Authorization: Bearer <INTERNAL_API_TOKEN>` (new env var, Worker secret + Pages env). The admin-app route delegates the real auth + body validation to the pure [server/newsletter/processWelcomeRequest.ts](manicbot/admin-app/src/server/newsletter/processWelcomeRequest.ts) (constant-time Bearer compare, email regex, lang fallback to `en`), then calls `sendNewsletterWelcomeEmail` (Resend transport) and stamps `welcome_sent_at`. On any failure path (token unset, ADMIN_APP_URL unset, admin-app non-200, fetch throw) the Worker stamps `welcome_send_error` on the subscriber row but the public endpoint still returns 202 — UX never regresses on misconfig, and email-enumeration via "this email already subscribed" is impossible because dedup + new-row both 202. CORS is open (`Access-Control-Allow-Origin *`); IP-rate-limited 60/min via the same shared D1 limiter as `/api/track`. Tests: [test/subscribe-http.test.js](manicbot/test/subscribe-http.test.js) (24 cases — parse/validation, dedup, rate-limit, 4 welcome-dispatch error paths, happy path, lang fold); [admin-app/src/__tests__/newsletter-welcome-template.test.ts](manicbot/admin-app/src/__tests__/newsletter-welcome-template.test.ts) (7 cases — 4-lang render, heading/body/bullets/unsubscribe URL); [admin-app/src/__tests__/newsletter-welcome-route.test.ts](manicbot/admin-app/src/__tests__/newsletter-welcome-route.test.ts) (18 cases — Bearer extraction, missing/wrong/constant-time-mismatch auth, body validation, happy path, sender error stamps `welcome_send_error`). Explicit follow-ups out of scope at 0086 ship time: ~~one-click unsubscribe flow~~ (shipped in 0090, see below), double-opt-in confirmation email (deliberately declined 2026-05-27 per user direction — single-opt-in only, the click on the landing form IS the consent record + we ship a real working unsub link in the welcome itself), broadcast surface that reads from this table.
- `0090_newsletter_unsubscribe_token.sql` — closes the 0086 follow-up: real one-click unsubscribe token for `newsletter_subscribers`. Pre-fix every landing-form welcome carried `?token=placeholder` — clicking 404'd. Adds nullable `unsubscribe_token TEXT` column, backfills existing rows with `lower(hex(randomblob(16)))` (SQLite cryptographic RNG, 32 hex chars / 128 bits, matches `marketing_contacts.unsubscribe_token` shape) BEFORE creating the partial UNIQUE `idx_newsletter_subscribers_unsub_tok` so the negligible collision risk can't fail the apply. Worker `subscribeHttp.js` mints a fresh token via the new pure helper `generateUnsubscribeToken()` in [src/http/subscribeHttpLogic.js](manicbot/src/http/subscribeHttpLogic.js) on every NEW-row INSERT and forwards it in the welcome-dispatch body to admin-app. **Resubscribe-after-unsub** path: `tryInsertSubscriber` (now returns `{kind: 'inserted'|'reactivated'|'noop', token?}` instead of `{inserted: boolean}`) detects a row with `unsubscribed_at IS NOT NULL` on the second submit, clears `unsubscribed_at + welcome_sent_at + welcome_send_error`, **keeps the original token** (stable across resub so any out-in-the-wild copy of the welcome email keeps working), and re-fires the welcome. Active-row resub stays a silent dedup no-op (current behaviour preserved, email-enumeration defense intact). Worker `unsubscribeHttp.js` (`GET /u/<token>`) extended with a fallthrough: tries `marketing_contacts` first (the per-tenant CRM, primary), then `newsletter_subscribers` — same handler serves both backends behind one URL. Added POST handler for **RFC 8058 one-click unsubscribe** — Gmail / Apple Mail render an inline "Unsubscribe" button in the message header and POST when clicked; the handler returns 204 + flips the row. [src/worker.js](manicbot/src/worker.js) accepts BOTH `GET` and `POST` on the `/u/` prefix. admin-app: [server/newsletter/processWelcomeRequest.ts](manicbot/admin-app/src/server/newsletter/processWelcomeRequest.ts) validates `unsubscribeToken` as `^[a-f0-9]{32}$` (400 on missing/malformed), forwards verbatim to `sendNewsletterWelcomeEmail`. [server/email/emailService.ts](manicbot/admin-app/src/server/email/emailService.ts) builds the real URL `${WORKER_PUBLIC_URL || 'https://manicbot.com'}/u/<token>` (matches the pattern used by [server/marketing/unsubscribeUrl.ts](manicbot/admin-app/src/server/marketing/unsubscribeUrl.ts)) and ships `List-Unsubscribe: <URL>` + `List-Unsubscribe-Post: List-Unsubscribe=One-Click` headers via the new optional `headers` param on `sendResendEmail`. CRM-mirror to `marketing_contacts` deliberately deferred — newsletter subscribers stay in their own table until the platform-broadcast surface ships (separate PR; `marketing_contacts.tenant_id` is NOT NULL so a mirror would need a sentinel `_platform` tenant or a schema change). Test coverage: [test/subscribe-http.test.js](manicbot/test/subscribe-http.test.js) (+9 cases: token shape contract, 32-hex on new-row, token forwarded in dispatch body, resub-after-unsub clears `unsubscribed_at` + keeps original token + re-fires welcome, active-row resub silent no-op), [test/unsubscribe-route.test.js](manicbot/test/unsubscribe-route.test.js) (+4 cases: newsletter fallthrough on GET, POST → 204, idempotency on second GET, `marketing_contacts` precedence when token matches both), [admin-app/src/__tests__/newsletter-welcome-route.test.ts](manicbot/admin-app/src/__tests__/newsletter-welcome-route.test.ts) (+4 cases: missing token → 400, wrong-shape token → 400, non-hex token → 400, token forwarded verbatim to `sendEmail` as 3rd arg), [admin-app/src/__tests__/newsletter-welcome-email-service.test.ts](manicbot/admin-app/src/__tests__/newsletter-welcome-email-service.test.ts) (NEW file, 5 cases: URL built with real token at `/u/`, RFC 8058 headers shipped, `WORKER_PUBLIC_URL` fallback to `manicbot.com`, trailing-slash strip, zero `placeholder` references in payload). Operator action: NONE — Worker auto-applies the migration on next `/admin/migrate-d1`, backfill is instant, existing welcome-email dispatches start using the real token on next request.
...
- `0090_chat_enabled.sql` — decouples the salon web-chat surface (`/salon/{slug}/chat`) from public-catalog publication. New column `tenants.chat_enabled INTEGER NOT NULL DEFAULT 1`. Backfilled with `UPDATE tenants SET chat_enabled = public_active` so the rollout is zero-behaviour-change for existing rows; new tenants get chat-on by default. **Before**: both the admin-app reader (`publicSalon.getProfile`) and the Worker `/chat/init` (`src/http/chatWebHttp.js loadSalonBranding` + `src/channels/resolver.js resolveTenantFromSlug`) filtered on `public_active = 1`. That coupled the chat URL — something the salon owner shares manually via printed QR / Instagram bio / business card — to listing the salon in the public catalog. **After**: the chat gate moves to `chat_enabled`; `publicActive` remains the catalog-visibility gate. The two decisions are now orthogonal. **New tRPC proc** `publicSalon.getProfileForChat` — clone of `getProfile` with `WHERE chatEnabled = 1` instead of `WHERE publicActive = 1`. Added to `PUBLIC_CACHEABLE_PROCEDURES` (60s edge cache, 5min SWR). **Extended** `salon.updateSalonProfile` zod input with `chatEnabled: z.number().min(0).max(1).optional()`; no publish-guard runs on chat-only writes (chat doesn't need slug+name+services to be enabled — natural gate is "no slug, no URL"). **Worker**: [src/tenant/storage.js](manicbot/src/tenant/storage.js) `putTenant` and `tenantRowToDoc` now carry the new column; the `INSERT OR REPLACE` column list grew by one. **UI rewrite**: [SalonChannelsTab.tsx](manicbot/admin-app/src/components/salon/SalonChannelsTab.tsx) `WebChatTab` is now a self-sufficient surface — takes only `tenantId`, fetches `salon.getSalonProfile` internally, renders an inline Setup card with slug + display name + logo (via existing `AssetUploadField kind="logo"`) + chat on/off toggle. The legacy "no slug → redirect to Public Profile tab" placeholder is gone. URL + QR show whenever `slug` is set; iframe preview shows whenever `slug && chatEnabled` regardless of `publicActive`. `SalonChannelsTab` signature shrank to just `{tenantId}`. Test coverage: [src/__tests__/publicSalon-get-profile-for-chat.test.ts](manicbot/admin-app/src/__tests__/publicSalon-get-profile-for-chat.test.ts) (3 cases — happy path, null-on-missing, static source-code pin on the WHERE column), [src/__tests__/salon-update-profile-chat-enabled.test.ts](manicbot/admin-app/src/__tests__/salon-update-profile-chat-enabled.test.ts) (5 cases — both values persisted, out-of-range rejected, publish-guard not triggered, field-omitted no-op), [test/chat-web-chat-enabled.test.js](manicbot/test/chat-web-chat-enabled.test.js) (2 cases — Worker source-code pins). Existing `publicSalon-fts-logic.test.ts` cache-allowlist test updated to include the new proc. Tenant-isolation allowlist line bumped 1752 → 1760.
...
| `dashboard/OnboardingChecklist.tsx` | Single setup checklist on the Overview tab. **Two-tier layout** (2026-05-27 rework): `ESSENTIAL_STEPS` (4 items — `connect_bot / add_master / set_master_schedule / add_service`, always visible — these map 1:1 to what blocks `getSlots()` from returning a bookable slot) + `OPTIONAL_STEPS` (4 items — `fill_salon_info / add_branding / activate_public / share_link`, collapsible under «Расширьте профиль»). Auto-opens the optional tier mid-flow (1..3 essentials done), closed at 0/4 + 4/4 to avoid nag; user toggle persists in localStorage `manicbot_onboarding_optional_collapsed`. Counter + progress bar use the essentials denominator. Headline flips «Настройте салон» → «✅ Готов принимать записи» at 4/4. Auto-hides the whole component only when all 8 ids are done. Closes three pre-rework routing bugs: `add_branding` → `/settings?section=salon` (cover edit lives there in `SalonSettingsEditor`, was `/settings?section=public`), `share_link` → `?tab=public_profile` (was `?tab=channels`), `set_master_schedule` label disambiguated (was «Настройте расписание» — read as salon-level hours, but the click target landed on the Masters tab). Dropped legacy ids: `invite_master` (renamed to `add_master`), `set_schedule` (renamed to `set_master_schedule`), `fill_description` (renamed to `fill_salon_info`), `add_logo` + `add_cover` (merged into `add_branding`, AND of both — a public card with only one looks broken), `first_booking` (vanity — was a counter, not a setup gate). Persisted JSON in `tenant_onboarding.completedSteps` keeps backward-compat: only `share_link` is read on the manual-mark path, dropped ids are silently ignored. Sidecar polish in [SalonSettingsEditor.tsx](manicbot/admin-app/src/components/salon/SalonSettingsEditor.tsx) adds a hint line under the salon hours from/to inputs clarifying that the field is shown on the public page; real booking slots come from each master's individual schedule. Pinned by `src/__tests__/OnboardingChecklist.test.tsx` (component, ~20 cases) + `src/__tests__/onboarding-router.test.ts` (router, 11 cases asserting the 8-id contract + add_branding AND-logic) + `src/__tests__/onboarding-logic.test.ts` (pure helpers). |
