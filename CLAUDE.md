...
- `0056_error_events.sql` — `error_events` table: deduped error tracker (fingerprint UNIQUE, severity/status lifecycle, tenant-scoped). Powers God Mode /errors page + ErrorStatsWidget on home dashboard. Write-path: `captureError()` in `src/utils/errorCapture.js`, wired into Worker fetch + scheduled via `ctx.waitUntil`.
