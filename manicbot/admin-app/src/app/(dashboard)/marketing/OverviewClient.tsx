...
function Metric({ label, value, icon: Icon }: { label: string; value: string | number; icon: any }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
      <div className="flex items-center gap-2 text-slate-400 text-[11px] uppercase tracking-wide font-semibold mb-1">
          ...
          {providersQ.data && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
              {providersQ.data.map((p: any) => (
                <div key={p.name} className="flex items-center justify-between rounded-lg bg-slate-950/40 border border-slate-800 px-3 py-2">
                  <div>
                    <div className="text-sm font-semibold text-slate-100 capitalize">{p.name}</div>
