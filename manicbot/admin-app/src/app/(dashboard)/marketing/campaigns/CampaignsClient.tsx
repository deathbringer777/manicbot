        ...
        {listQ.isLoading ? (
          ...
          <ul className="space-y-1.5 mt-2">
            {listQ.data.map((c: any) => (
              <li key={c.id} className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 px-3 py-2 text-xs">
                <div>
                ...
                <span className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-400">{c.status}</span>
              </li>
            ))}
          </ul>
        ) : (
          <div className="text-center py-8 text-slate-500">
            ...
            <div className="text-sm text-slate-700 dark:text-slate-400 font-medium mb-1">No campaigns yet</div>
