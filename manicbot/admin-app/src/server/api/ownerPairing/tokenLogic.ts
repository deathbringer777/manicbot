/**
 * Shared helper for the owner Telegram-pairing tRPC procedures.
 *
 * Mirror of `manicbot/src/services/ownerPairing.js` (Worker side) —
 * both sides MUST produce the same SHA-256 hash for a given raw token
 * so the bot's `/start own_<raw>` consumer can look up the row by
 * hash. Pure-functional pieces here are unit-tested in
 * `__tests__/owner-pairing-token-logic.test.ts`.
 *
 * The DB insert is intentionally NOT factored out — the tRPC
 * procedure writes the row directly with `ctx.webUser.id` attribution.
 */
 ...
 * caller can hand `raw` to the owner (in the deep-link) and persist
 * only `hash`.
 */
...
/**
 * Build the Telegram deep-link URL. `own_` prefix mirrors the Worker
