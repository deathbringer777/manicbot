 ...
 *
 * Host depends on the token generation:
 *  - instagram_direct (IGAA / Instagram Login) → graph.instagram.com
 *  - legacy (Basic Display)                    → graph.facebook.com
...
export async function refreshInstagramToken(ctx, tenantId, channelConfigId, encKey) {
  ...
  const rows = await dbAll(ctx,
    'SELECT token_encrypted, config FROM channel_configs WHERE id = ? AND tenant_id = ? LIMIT 1',
  ...
  const cfg = (() => { try { return JSON.parse(rows[0].config ?? '{}'); } catch { return {}; } })();
  const refreshHost = cfg.api === 'instagram_direct'
