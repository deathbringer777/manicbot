export default {
  // Support tickets (general)
  support_enter_msg: '🆘 Напишите ваше сообщение в поддержку (одним сообщением):',
  support_claim_btn: '✋ Взять тикет',
  support_registered: '✅ Ты добавлен в поддержку. Будешь получать уведомления о новых тикетах и сможешь нажимать «Взять тикет».',
  support_only_admin: '❌ Только админ может добавлять или удалять агентов поддержки.',
  support_add_usage: 'Использование: /add_support @username или /add_support <ID>',
  support_remove_usage: 'Использование: /remove_support @username или /remove_support <ID>',
  support_user_not_found: '❌ Пользователь не найден. Укажи @username или ID.',
  support_removed: '✅ <b>{n}</b> снят с роли поддержки.',
  support_local_created: '✅ Ваш вопрос принят. Мастер свяжется с вами.',

  // Consultant / live chat
  consultant_btn: '👤 Подключить консультанта',
  consultant_btn_hint: '💬 Нажмите кнопку — к вам подключится мастер или админ. В режиме чата пишите сообщения, они дойдут до специалиста. Чтобы завершить — напишите СТОП.',
  consultant_sent: '✅ Заявка передана консультанту. Ожидайте ответа в ближайшее время.',
  consultant_notify: '👤 <b>Клиент просит консультанта</b>\n\n👤 {name}\n📱 {phone}\n🔗 {username}\n\nПодключитесь или отклоните — клиент ждёт.',
  consultant_constructive: '🙏 Давайте общаться конструктивно. Если нужна помощь — нажмите /start или кнопку «Подключить консультанта».',

  // Instagram optional AI gate (INSTAGRAM_AI_TRIGGER secret) — do not list actual triggers
  ig_ai_trigger_hint: '💬 Чтобы ассистент ответил, напишите сообщение с ключевым словом из описания профиля салона или закреплённой публикации (например про запись или вопрос об услугах).',

  // Ticket flow
  ticket_desc: '💬 <b>Режим чата с консультантом</b>\n\nВы подключаетесь к мастеру или админу. Бот перейдёт в режим чата: ваши сообщения будут приходить специалисту, ответы — вам. Чтобы завершить — напишите СТОП (или STOP).',
  ticket_taken_by: '✅ С вами подключился <b>{name}</b>. Пишите сообщения — они дойдут до мастера.',
  ticket_taken_else: '⚠️ Тикет уже взял другой агент.',
  ticket_cannot_take_own: '⚠️ Нельзя взять свой же тикет. Дождитесь, пока другой мастер или админ возьмёт его.',
  ticket_take_btn: '👤 Взять в работу',
  ticket_decline_btn: '❌ Отклонить',
  ticket_close_btn: '🔒 Закрыть тикет',
  ticket_closed: '🔒 Тикет закрыт. Вы снова в обычном режиме бота.',
  ticket_closed_master: '🔒 Тикет закрыт. Клиент снова в обычном режиме.',
  ticket_declined: '🙏 Извините, сейчас все специалисты заняты. Попробуйте через 30 минут или нажмите /start для записи.',
  ticket_from_client: '💬 <b>Клиент:</b>\n{msg}',
  ticket_forwarded_ok: '✅ Сообщение передано специалисту. Ожидайте ответа здесь. Чтобы выйти из чата с консультантом — напишите СТОП.',
  ticket_reply_sent: '✅ Отправлено клиенту.',
  ticket_master_hint: '💬 Чат с клиентом. Пишите сообщения в этот чат — они уйдут клиенту. Нажмите «Закрыть тикет», когда вопрос решён.',
  ticket_internal_note: '📋 <i>Контекст диалога (только для мастера/админа):</i>\n\n{note}',

  // Free correction
  ticket_free_correction_btn: '🔧 Бесплатная коррекция',
  correction_offer_msg: '🔧 <b>Мастер предлагает бесплатное исправление</b>\n\nНажмите кнопку, чтобы выбрать дату и время:',
  correction_book_btn: '📝 Записаться на исправление',

  // Technical support (platform level)
  tech_support_enter_msg: '🔧 Опишите проблему (одним сообщением). Ответит агент техподдержки платформы:',
  tech_support_created: '✅ Ваш запрос передан в техническую поддержку платформы.',
  tech_support_only_staff: '❌ Обращаться в техподдержку могут только мастера и владельцы салонов.',
  tech_support_notify: '🔧 <b>Запрос техподдержки</b>\n🏢 {salon}\n👤 {name} ({role})\n\n{msg}',
};
