export default {
  reg_confirm_name: '📝 Для запису потрібна реєстрація.\n\nТвоє ім\'я в Telegram: <b>{n}</b>\n\nПродовжуючи, ти приймаєш <a href="https://manicbot.com/rules">Правила користування</a>.\n\nВсе вірно?',
  reg_yes: '✅ Приймаю та продовжую',
  reg_change: '✏️ Змінити ім\'я',
  reg_enter_name: '✏️ Введи своє ім\'я:',
  reg_name_err: '❌ Введи коректне ім\'я (2-50 символів):',
  reg_name_invalid: '🙂 Схоже, це не ім\'я. Напиши, будь ласка, як тебе звати — наприклад, Анна:',
  reg_phone: 'Чудово, <b>{n}</b>! 😊\n\n📱 Введи номер телефону або натисни кнопку:\n\n<i>Будь ласка, перевірте правильність номера</i>',
  // Channels without a Telegram reply-keyboard (web/Instagram/WhatsApp): no button to press.
  reg_phone_web: 'Чудово, <b>{n}</b>! 😊\n\n📱 Напиши номер телефону у відповідь (наприклад, +48123456789):\n\n<i>Будь ласка, перевірте правильність номера</i>',
  reg_phone_btn: '📱 Надіслати номер',
  reg_phone_err: '❌ Введи коректний номер телефону:',
  reg_done: '✅ <b>Реєстрація завершена!</b>\n\n👤 Ім\'я: <b>{n}</b>\n📱 Телефон: <b>{p}</b>',
  now_choose: '💅 Тепер обери послугу:',

  // Email capture (chat → marketing)
  email_ask: '✉️ <b>Хочеш стати постійним клієнтом?</b>\n\nЗалиш email — і першим отримуй знижки, акції та новини салону. Відписатися можна будь-коли.',
  email_ask_btn: '✉️ Залишити email',
  email_decline_btn: 'Не зараз',
  email_ask_prompt: '✍️ Напиши свій email одним повідомленням:',
  email_invalid: '❌ Це не схоже на email. Спробуй ще раз або натисни «Не зараз».',
  email_thanks: '🎉 <b>Дякуємо!</b>\n\nНадсилатимемо лише корисне — знижки та новини. Відписатися можна будь-коли в меню.',
  email_declined: '👌 Гаразд! Якщо передумаєш — email завжди можна залишити пізніше.',
  email_optout_btn: '🔕 Відписатися від розсилки',
  email_optout_done: '✅ Готово. Більше не надсилатимемо листи. Захочеш повернутися — просто залиш email знову.',

  choose_svc: '💅 <b>Обери послугу:</b>',
  choose_date: '📅 Обери дату:',
  no_slots: '😔 На <b>{d}</b> немає вільних місць.\n\n📅 Обери іншу дату:',
  choose_time: '🕐 Обери час:',
  other_svc: '◀️ Інша послуга',
  other_date: '◀️ Інша дата',
  chosen: '✅ Обрано: <b>{svc}</b>\n💵 {p} {c} · ⏱ {d} {min}',
  chosen_correction: '✅ Обрано: <b>{svc}</b>\n\nОбери дату:',
  confirm_correction: '📋 <b>Підтвердження запису</b>\n\n{svc}\n📅 {dt}\n\n👤 {name}\n📱 {phone}',
  free_label: 'Безкоштовно',

  book_choose_master: '👩‍🎨 <b>Вибери майстра</b>\n\nМожеш вибрати конкретного майстра або записатися до будь-якого вільного.',
  book_any_master: '🎲 Будь-який вільний майстер',
  book_master_label: '👤 {name}',
  book_master_assigned: '👩‍🎨 Майстер: <b>{name}</b>',
  book_any_master_label: '🎲 Будь-який майстер',

  confirm_title: '📋 <b>Підтвердження запису</b>',
  confirm_yes: '✅ Підтвердити',
  confirm_no: '❌ Скасувати',
  book_confirm_declined: '❌ <b>Підтвердження знято.</b>\n\n📅 Слот збережено: <b>{dt}</b>\n\nНапишіть послугу (наприклад «педикюр») або оберіть зі списку.',
  book_repick_service: '💅 Обрати послугу зі списку',
  book_choose_svc_adjust: '💅 Оберіть послугу — дата й час залишаються ті самі.',
  booked: [
    '🎉 <b>Запис підтверджено!</b>', '',
    '', '{svc}', '',
    '📅 {dt}', '',
    '⏱ {dur} {min}', '💵 {p} {c}', '',
    '📍 {addr}', '{maps}', '',
    '⏰ Нагадаю тобі:', '• За 24 години', '• За 2 години',
    '', '📅 Додати в календар (Google Calendar/Mac) ⬇️'
  ],
  booked_correction: [
    '🎉 <b>Запис підтверджено!</b>', '',
    '', '{svc}', '',
    '📅 {dt}', '',
    '📍 {addr}', '{maps}', '',
    '⏰ Нагадаю тобі:', '• За 24 години', '• За 2 години',
    '', '📅 Додати в календар (Google Calendar/Mac) ⬇️'
  ],
  book_cancelled: '❌ Запис скасовано.\n\nОбери, що тебе цікавить:',
  book_err: '❌ Помилка. Почни запис спочатку.',
  book_limit: '⚠️ Досягнуто ліміт записів ({n}). Скасуй одну з поточних.',
  slot_taken: '😔 Цей час щойно зайняли. Обери інший:',
  apt_blocked_no_show: '⚠️ На жаль, онлайн-запис зараз недоступний через пропущені візити. Будь ласка, зв’яжіться з салоном напряму, щоб записатися.',

  my_title: '📋 <b>Мої записи</b>',
  my_empty: 'У тебе немає майбутніх записів.\n\n💅 Хочеш записатися?',
  my_cancel: '❌ Скасувати: {d} {t}',
  my_cancel_all: '🗑 Скасувати всі записи',
  cancel_confirm: '⚠️ Точно скасувати?\n\n{svc}\n📅 {dt}',
  cancel_all_confirm: '⚠️ Скасувати всі {n} записів?',
  cancel_yes: '❌ Так, скасувати',
  cancel_all_yes: '🗑 Так, скасувати всі',
  cancel_no: '◀️ Ні, назад',
  cancel_all_ok: '✅ Всі записи скасовано.',
  cancel_comment_prompt: '💬 Додай коментар до скасування для майстра/адміна\nабо натисни «Пропустити»:',
  cancel_comment_skip: '⏭ Пропустити',
  cancel_ok: '✅ <b>Запис скасовано:</b>\n\n{svc}\n📅 {dt}\n\nХочеш перезаписатися на іншу дату?',
  cancel_err: '❌ Запис не знайдено або вже скасовано.',
  rebook: '📝 Записатися знову',

  rem_24: ['⏰ <b>Нагадування!</b>','','Завтра у тебе запис:','','','{svc}','','📅 {dt}','','📍 {addr}','{maps}','','До зустрічі! 💅'],
  rem_2:  ['⏰ <b>Нагадування!</b>','','Через 2 години:','','','{svc}','','📅 {dt}','','📍 {addr}','{maps}','','Вже скоро! 💖'],
  staff_apt_cancelled_client: '❌ <b>Запис скасовано клієнтом</b>',

  // Reviews
  review_request: '💬 <b>Дякуємо за візит!</b>\n\nЯк вам обслуговування? Поставте оцінку:',
  review_thanks: '✅ Дякуємо за оцінку! ({rating}⭐)\n\nБажаєте залишити коментар?',
  review_add_comment: '✏️ Так, написати',
  review_skip_comment: '⏭ Пропустити',
  review_enter_text: '✏️ Напишіть ваш відгук:',
  review_text_saved: '✅ Відгук збережено!\n\nБажаєте додати фото? (до 3 шт.)',
  review_add_photo: '📸 Додати фото',
  review_done: '⏭ Готово',
  review_send_photo: '📸 Надішліть фото (ще {n}):',
  review_photo_saved: '✅ Фото додано! ({count}/3)',
  review_complete: '🎉 <b>Дякуємо за відгук!</b>\n\n{rating}⭐\n{text}',
  review_complete_no_text: '🎉 <b>Дякуємо за відгук!</b>\n\n{rating}⭐',
  review_already: 'Ви вже залишали відгук для цього запису.',
};
