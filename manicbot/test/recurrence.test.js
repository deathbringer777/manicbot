...
describe('expandOccurrences', () => {
  ...
  const to = new Date(Date.UTC(2026, 4, 24, 23, 59, 59)); // inclusive end of Sunday May 24
  ...
  it('once: returns empty when anchor before window', () => {
    ...
    const occs = expandOccurrences({ type: 'once' }, anchor, earlier, to);
  ...
  it('weekly Mon/Wed/Fri: emits 3 across the week', () => {
    ...
    expect(occs.map((d) => d.toISOString())).toEqual([
      '2026-05-18T09:00:00.000Z', // Mon
      '2026-05-20T09:00:00.000Z', // Wed
      '2026-05-22T09:00:00.000Z', // Fri
    ]);
  });
  ...
  it('monthly_day: only the matching day', () => {
    ...
    const to2 = new Date(Date.UTC(2026, 7, 1, 0, 0, 0)); // May 1 -> Aug 1
    const occs = expandOccurrences(
  ...
  it('honors until cutoff', () => {
    ...
    expect(occs).toHaveLength(3); // Mon, Tue, Wed (inclusive of 'until' day)
...
describe('nextOccurrenceAfter', () => {
  ...
  it('daily: returns next day at fire time if today already fired', () => {
    const after = new Date(Date.UTC(2026, 4, 18, 10, 0, 0)); // Mon 10:00, fire time 09:00 already past
  ...
  it('daily: returns today if fire time still in future', () => {
    const after = new Date(Date.UTC(2026, 4, 18, 8, 0, 0)); // Mon 08:00, fire time 09:00 still ahead
    const next = nextOccurrenceAfter({ type: 'daily', time: '09:00' }, '2026-05-18', after);
    expect(next?.toISOString()).toBe('2026-05-18T09:00:00.000Z');
  });

  it('weekly Mon/Wed/Fri: skips to next matching weekday', () => {
    const after = new Date(Date.UTC(2026, 4, 18, 10, 0, 0)); // Mon 10:00
    const next = nextOccurrenceAfter(
      { type: 'weekly', time: '09:00', weekdays: [1, 3, 5] },
      '2026-05-18', after,
    );
    expect(next?.toISOString()).toBe('2026-05-20T09:00:00.000Z'); // Wed
  });

  it('returns null past until cutoff', () => {
    const after = new Date(Date.UTC(2026, 4, 21, 0, 0, 0)); // Thu, past until
    const next = nextOccurrenceAfter(
      { type: 'daily', time: '09:00', until: '2026-05-20' },
      '2026-05-18', after,
    );
